    ...
    if (role !== "admin") {
      const chatUser = await db.ChatUser.findOne({
        where: {
          userId,
          chatId,
          status: { [Op.in]: ["active", "invited", "suspend"] },
        },
      });

      if (!!chatUser) {
        if (chatUser.status !== "suspend") {
...
io.on("connection", (socket) => {
  ...
  socket.on("message", async (message, callback) => {
    ...
    checkMessage(message, socket.user.id, socket.chat.id)
      .then((message) => {
  ...
  if (
    ...
    socket.chat.createdBy === socket.user.id
  ) {
