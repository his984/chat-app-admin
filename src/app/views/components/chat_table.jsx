...
export function ChatTable({
  ...
  const getChats = () => {
    ...
    get(`${endpoints.chats.base}?page=${chatsPage}`)
      .then((data) => {
        console.log(data);
        if (slice === "chats") dispatch(updateChats(data));
        else dispatch(setChats(data.rows));
      })
      .finally(() => {
        afterLoad();
      });
    ...
    <>
      ...
      <Table>
        ...
        <Table.Body className="divide-y">
          <>
            {[...(slice === "chats" ? chatsPagination.rows : chatsPagination)]
              .sort((a, b) => {
              ...
              .map((chat) => {
                return (
                  ...
                  >
                    ...
                    <Table.Cell>
                      {chat.status === "active" || chat.status === "suspend" ? (
                        <Link
                          to={`/chats/${chat.id}`}
