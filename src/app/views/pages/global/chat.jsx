...
export function Chat() {
    ...
    return <AuthLayout>
        ...
        {
            showAddUsersModal ?
                <AddUsersModal show={showAddUsersModal} onClose={() => {
        ...
        {
            currentUserAction.id ? <UserActionsModal chatId={chat.id} show={showCurrentUserAction} onClose={() => {
                setShowCurrentUserAction(false)
            }} user={currentUserAction} socket={window.socket}/> : <></>
        }
...

        {
            isLoading ? <div className='flex flex-row justify-center items-center h-[20rem]'>
                ...
                <div className='py-4'>
                    <div className={` ${active !== 0 ? 'hidden' : ''} flex flex-row justify-center items-center`}>
                        <div className="container mx-auto">
                            <div
                                className=" border border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-gray-800 rounded-lg">
                                <div>
                                    <div className="">
                                        <div
                                            className="flex justify-between p-3 border-b border-gray-300 text-blue-600 dark:border-gray-700">
                                            <div
                                                className="relative flex items-center items-center">
                                                <img
                                        ...
                                        <div
                                            className="relative w-full p-6   h-[28rem] md:h-[30rem]  overflow-y-auto flex flex-col-reverse ">
                                            <ul className="space-y-2">
                                                {
                                                    messages.map((message) => {
                                                        return (
                                                            <li key={message.id + Math.random()}
                                                                className={`flex items-end ${message.sender.id === currentUser.id ? 'justify-end' : 'justify-start'}`}>
                                                                ...
                                                                <div className="flex flex-col">
                                                                    ...
                                                                    <div
                                                                        className={`relative max-w-xs px-6 py-2 text-white ${message.sender.id === currentUser.id ? (message.code && message.code !== 200 ? 'bg-red-700' : 'bg-blue-700') : 'bg-gray-500 dark:bg-gray-700'}   rounded-full shadow`}>
                                                                        <span className="block">{message.content}</span>
                                                                    </div>
                                                                    {message.code && message.code !== 200 ? <span
                                                                        className="self-end px-6 text-xs py-1 text-red-500">
                                                                          {message.description}
                                                                        </span> : <></>}
                                                                </div>
                                                                {
                                        ...
                                        <div
                                            className="flex items-center justify-between w-full p-3 border-t border-gray-300 dark:border-gray-700">
